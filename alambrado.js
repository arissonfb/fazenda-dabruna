...
function seedAlambradoDemoRecords() {
  const farms = getAllFarms();
  if (!farms.length) return;

  const already = farms.reduce((sum, farm) => sum + getAlambradoRecords(farm).length, 0);
  if (already > 0) return;

  const observacoesPool = [
    "Manutenção preventiva realizada conforme cronograma.",
    "Substituição de trecho danificado por queda de árvore.",
    "Reforço solicitado após identificação de fuga de animais.",
    "Serviço executado em parceria com equipe terceirizada.",
    "Ajuste realizado durante rotina de manejo do potreiro.",
    "Reparo emergencial após temporal na região.",
    "Melhoria estrutural para divisão de novo lote.",
    ""
  ];
  ...
  while (seeded < 30 && guard < 500) {
    guard++;
    const farm = farms[farmIndex % farms.length];
    farmIndex++;
    ...
    const quantidade = tipo.medida === "metro"
      ? Math.round((40 + Math.random() * 460) / 5) * 5
      : Math.max(1, Math.round(1 + Math.random() * 9));
    ...
    getAlambradoRecords(farm).push({
      ...
      codigo: generateAlambradoCode(farm),
      farmId: farm.id,
      potreiroId: potreiro ? potreiro.id : "",
      potreiroName: potreiro ? potreiro.name : "",
      tipoServicoId: tipo.id,
